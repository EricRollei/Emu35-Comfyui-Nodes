...
To save VRAM, you can quantize the models to 4-bit (NF4) using the included script. This allows you to load the model faster and with less memory usage in future runs.

1.  **Open a terminal** in the `ComfyUI/custom_nodes/emu35` directory.

2.  **Run the quantization script:**

    **For the Image Model:**
    ```bash
    python quantize.py --model_path "../../models/emu35/Emu3.5-Image" --output_path "../../models/emu35/Emu3.5-Image-nf4" --quant_type nf4
    ```

    **For the Base (VQA) Model:**
    ```bash
    python quantize.py --model_path "../../models/emu35/Emu3.5-Base" --output_path "../../models/emu35/Emu3.5-Base-nf4" --quant_type nf4
    ```

3.  **Advanced Options:**
    *   **Skipping Modules:** If you notice quality degradation, you can skip quantizing the output layer (`lm_head`) by adding `--skip_modules lm_head`. This increases the model size by ~1.5GB but may preserve better detail.
        ```bash
        python quantize.py ... --skip_modules lm_head
        ```

4.  **Usage:**
    After quantization, refresh ComfyUI. The new folders (e.g., `Emu3.5-Image-nf4`) will appear in the Loader node. Select the new folder and set **precision** to `nf4`.
