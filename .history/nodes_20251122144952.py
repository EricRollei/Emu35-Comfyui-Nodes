...
class Emu35Sampler:
    ...
    def generate(self, model, tokenizer, vq_model, prompt, negative_prompt, width, height, steps, cfg_scale, seed, input_image=None):
        ...
        try:
            ...
            # Access the internal tiktoken encoding if possible, or just catch the error
            # Since we can't easily pass errors='ignore' to the top level decode if it doesn't support it (it should though)
            # The traceback showed it supports errors.
            # Let's try to manually decode if the standard way fails, or just rely on the fix in tokenization_emu3.py
            raise e
