...
class Emu35Sampler:
    ...
    def generate(self, model, tokenizer, vq_model, prompt, negative_prompt, width, height, steps, cfg_scale, seed, input_image=None):
        ...
        # Streamer for progress bar - throttled to avoid CPU bottleneck
        # Only update every 64 tokens (1 row)
        ...
        streamer = ThrottledStreamer(generation_config.max_new_tokens)
        ...
        try:
            ...
            if decoded_text is None:
                ...
                fallback_ids = [tok for tok in generated_tokens[0].tolist() if tok in getattr(tokenizer, "decoder", {})]
                if fallback_ids:
