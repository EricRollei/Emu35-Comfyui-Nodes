...
def decode_image(image_string, tokenizer, vision_tokenizer):
    ...
    image_rows = re.split(re.escape(eol_token), image_string)
...
class Emu35Loader:
    ...
    def load_model(self, model_name, vq_model_name, precision):
        ...
        # Try to compile the model for faster inference (Windows support varies)
        # Only compile the forward pass, not the whole generate loop
        # if os.name != 'nt': # Skip on Windows by default unless user wants to try
        #     try:
