...
class Emu35Sampler:
    ...
    def generate(self, model, tokenizer, vq_model, prompt, negative_prompt, width, height, steps, cfg_scale, seed, input_image=None):
        ...
        # Prepare Inputs
        # Emu 3.5 prompt formatting
        # Template: <|extra_203|>You are a helpful assistant. USER: {prompt} ASSISTANT: <|extra_100|>
        
        system_prompt = "You are a helpful assistant. "
        full_prompt = f"<|extra_203|>{system_prompt}USER: {prompt} ASSISTANT: <|extra_100|>"
        full_negative_prompt = f"<|extra_203|>{system_prompt}USER: {negative_prompt} ASSISTANT: <|extra_100|>"
        ...
        # Calculate latent dimensions (Emu3 uses 16x compression)
        ...
        try:
            ...
            print("Continuing without custom logits processor for better speed.")
        ...
        try:
            # Extract text response (before BOI)
            boi_marker = "<|begin_of_image|>"
            boi_idx = decoded_text.find(boi_marker)
            if boi_idx != -1:
                text_response = decoded_text[:boi_idx].strip()
                # Clean up special tokens from text
                text_response = text_response.replace("<|extra_203|>", "")
                text_response = text_response.replace("<|extra_100|>", "")
                text_response = text_response.replace("<|extra_204|>", "")
