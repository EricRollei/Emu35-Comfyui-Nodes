...
class Emu35Sampler:
    ...
    def generate(self, model, tokenizer, vq_model, prompt, negative_prompt, width, height, steps, cfg_scale, seed, input_image=None):
        ...
        # Prepare Inputs
        # Emu 3.5 image generation prompt format
        # For image generation, we don't use the chat format. Instead, we use:
        # <|extra_203|>{prompt}<|extra_204|>{height}*{width}<|image|>
        # This tells the model to generate an image directly.
        
        # Calculate latent dimensions first (needed for prompt)
        latent_height = height // 16
        latent_width = width // 16
        
        # Format: BOS + prompt + EOS + resolution + IMG token
        full_prompt = f"<|extra_203|>{prompt}<|extra_204|>{latent_height}*{latent_width}<|image|>"
        full_negative_prompt = f"<|extra_203|>{negative_prompt if negative_prompt else ''}<|extra_204|>{latent_height}*{latent_width}<|image|>"
