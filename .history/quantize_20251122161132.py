...
def build_max_memory(cpu_memory_gb):
    bytes_per_gib = 1024 ** 3
    max_memory = {"cpu": cpu_memory_gb * bytes_per_gib}
    if torch.cuda.is_available():
        for idx in range(torch.cuda.device_count()):
            props = torch.cuda.get_device_properties(idx)
            total_gb = math.floor(props.total_memory / bytes_per_gib)
            usable_gb = max(total_gb - 4, 8)
            max_memory[idx] = usable_gb * bytes_per_gib
    return max_memory


def quantize_model(model_path, output_path, quant_type="nf4", skip_modules=None, cpu_memory_gb=256):
    ...
    # Prefer the native Emu3 class (uses local repo code), but fall back to AutoModel if needed
    candidate_loaders = []
    if EmuModelClass is not None:
        candidate_loaders.append(("Emu3ForCausalLM", EmuModelClass))
    ...
    if EmuModelClass is not None:
        try:
            ...
            config = None
    if config is not None and not hasattr(config, "mlp_bias"):
    ...
    base_load_kwargs = dict(
        ...
        max_memory=build_max_memory(cpu_memory_gb),
    )

    for loader_name, loader_cls in candidate_loaders:
        print(f"Trying {loader_name} with streaming load...")
        try:
            if loader_name == "AutoModelForCausalLM" and config is not None:
                model = loader_cls.from_pretrained(model_path, config=config, **base_load_kwargs)
            else:
                model = loader_cls.from_pretrained(model_path, **base_load_kwargs)
            print(f"Successfully loaded with {loader_name}.")
            break
        except Exception as e:
            load_attempts.append((loader_name, str(e)))
            print(f"{loader_name} load failed: {e}")

    if model is None:
        print("Error loading model; tried the following loaders:")
        for name, err in load_attempts:
            print(f"  - {name}: {err}")
