...
def quantize_model(model_path, output_path, quant_type="nf4", skip_modules=None):
    ...
    # Patch config if mlp_bias is missing
    config = None
    if EmuModelClass is not None:
        try:
            ...
            if not hasattr(config, "mlp_bias"):
                ...
                print("Patched config: added mlp_bias=False")
        except Exception as e:
            print(f"Warning: Could not load/patch config: {e}")
            config = None
    ...
    try:
        if config is not None:
            model = AutoModelForCausalLM.from_pretrained(model_path, config=config, **base_load_kwargs)
        else:
    ...
    # Copy all necessary tokenizer files
    tokenizer_files = [
    ...
    for filename in tokenizer_files:
        try:
            # Try repo first, then model folder
            source_file = None
            if filename == "tokenization_emu3.py":
                repo_file = os.path.join(emu_repo_path, "src", "tokenizer_emu3_ibq", filename)
